...
var $ = (function () {
    ...
    $.array = function (ls) {
        ...
        if ($.type(ls.item)   === "function" &&
            $.type(ls.length) === "number") {
    ...
    $.getClientPosition = function (element, parent) {
    ...
    $.scrollTo = function (child, parent) {
        var offset = $.getClientPosition(child, parent);
        if (parent) {
            parent.scrollTop = offset.y;
            parent.scrollLeft = offset.x;
        } else {
